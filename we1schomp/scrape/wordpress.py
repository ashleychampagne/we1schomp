...
def check_for_api(site):
    ...
    if not config.CONFIG['WORDPRESS_ENABLE'] or site.get('skip', False):
        log.warning(_('Skipping: %s'), site['name'])
        return False
    if not site.get('wpEnable', config.CONFIG['WORDPRESS_ENABLE']):
        log.warning(_('WordPress disabled: %s'), site['name'])
        return False

    log.info(_('Testing WordPress API for site: %s'), site['name'])
    wp_url = f"http://{site['site'].strip('/')}{config.CONFIG['WORDPRESS_API_URL']}"
...
def yield_query_results(site):
    ...
    pages_ok = config.CONFIG['WORDPRESS_GET_PAGES'] and site.get('wpPagesEnable', True)
    posts_ok = config.CONFIG['WORDPRESS_GET_POSTS'] and site.get('wpPostsEnable', True)
    ...
    for query in site.get('queries', config.CONFIG['QUERIES']):
        ...
        if not pages_ok:
            ...
            wp_query = config.CONFIG['WORDPRESS_PAGES_QUERY_URL'].format(
                api_url=site['wpURL'],
                query=query.replace(' ', '+'))
        ...
        if not posts_ok:
            ...
            wp_query = config.CONFIG['WORDPRESS_POSTS_QUERY_URL'].format(
                api_url=site['wpURL'],
                query=query.replace(' ', '+'))
...
def save_articles(site):
    ...
    for query, query_result in yield_query_results(site):
        ...
        article = dict(
            ...
            name=slug,
            metapath=config.CONFIG['DB_METAPATH'].format(site=site['slug']),
            pub=site['name'],
            pub_short=site['slug'],
